# EcoSynTech PCB v6.3 Final — SCHEMATIC NETLIST v3 (FINAL)
# Based on original docx spec | Date: 2026-04-15
# ==============================================================
# All fixes from deep review applied:
# V2→V3 fixes:
# 1. Added TPL5010 timing resistor R_WD_RT (47kΩ/49.9kΩ for ~1.6s timeout)
# 2. Added POWER_GOOD circuit (MCP809T-315 voltage supervisor)
# 3. Added BOOT_OK pull-up (firmware-controlled GPIO with pull-up)
# 4. Added RELAY_EN hardware interlock circuit (firmware + hardware)
# 5. I2C pull-ups consolidated: all on +3V3_ANA (per spec B7/B15)
# 6. ADS1115 redundant pull-ups removed (ESP32 I2C pull-ups serve full bus)
# 7. ADS1115 ALERT pin properly handled (NC per spec, can be routed to ESP32 GPIO)
# 8. BOOT_OK defined as ESP32 GPIO1 output (after firmware boot)
# 9. All components verified against spec line-by-line

# ==============================================================
# POWER ARCHITECTURE
# 12V_IN → F1(2A) → GDT1/MOV1 → R_SURGE/L_SURGE → TVS1 → Q_PROTECT → +12V_PROTECTED
#   → MP1584#1 → +5V_MAIN → D_MAIN → +5V_SYS → USB OR-ing via D_USB
...
F1     | Fuse Slow-Blow 2A 250V     | FUSE_HOLDER_5x20 | (1):+12V_IN, (2):+12V_FUSED
...
L_SURGE1| 22uH >=2A (optional)      | RADIAL_8x10     | (1):NET_SURGE_NODE, (2):+12V_SURGE
...
# ==============================================================
# SHEET 3: BUCK REGULATOR 12V→3.3V (MP1584 #2) — per spec B4
# ==============================================================
...
L_3V3  | 10uH >=2A Low-DCR         | SMD_6x6         | (1):SW_3V3, (2):+3V3_MAIN
...
# ==============================================================
# SHEET 4: RAIL SPLITTING + POWER_GOOD — per spec B6, B8
# ==============================================================
...
# POWER_GOOD CIRCUIT — monitors +3V3_ESP, outputs HIGH when stable
U_PWR_GOOD | Voltage Supervisor MCP809T-315 | SOT-23          | VCC:+3V3_ESP, GND:GND_STAR, OUT:POWER_GOOD
C_PWR_GOOD | 100nF 10V X7R            | C_0805          | (+):+3V3_ESP, (-):GND_STAR
# Note: POWER_GOOD drives LED_PWR (via R_LED_PWR) and hardware relay interlock
...
# ==============================================================
# Module: ESP32-WROOM-32E (replaces deprecated ESP32-WROOM-32 per user request)
# Footprint: ESP32_WROOM_38P (pin-compatible with ESP32-WROOM-32)

U_ESP32| ESP32-WROOM-32E            | ESP32_WROOM_38P  | 
        |                             |                  | VDD3.3 → +3V3_ESP
        |                             |                  | GND → GND_STAR
        |                             |                  | EN → EN_ESP (auto-reset via Q_RST + TPL5010 WATCHDOG_RST)
        |                             |                  | IO0(BOOT) → BOOT_ESP (auto-reset via Q_BOOT)
        |                             |                  | IO1(TXD) → UART0_TX → CP2102 RX; also = BOOT_OK output (firmware)
        |                             |                  | IO3(RXD) → UART0_RX → CP2102 TX
        |                             |                  | IO21 → I2C_SDA (pull-up 4.7k → +3V3_ANA)
        |                             |                  | IO22 → I2C_SCL (pull-up 4.7k → +3V3_ANA)
        |                             |                  | IO26 → RELAY1_DRV → Q_R1 base
        |                             |                  | IO27 → RELAY2_DRV → Q_R2 base
        |                             |                  | IO25 → RELAY3_DRV → Q_R3 base
        |                             |                  | IO14 → RELAY4_DRV → Q_R4 base
        |                             |                  | IO33 → DHT22_RAW (via 100Ω R_DHT_SER)
        |                             |                  | IO32 → DS18B20_RAW (via 100Ω R_DS_SER)
        |                             |                  | IO4 → WATCHDOG_KICK → TPL5010 DONE
        |                             |                  | IO16 → LED_WIFI_CTRL (active-LOW, sinks current)
        ...
        |                             |                  | IO36 → NC (reserved test pad)
        |                             |                  | IO39 → NC (reserved test pad)
...
C_ESP_DEC1| 100nF 10V X7R           | C_0805          | (+):+3V3_ESP, (-):GND_STAR
C_ESP_DEC2| 100nF 10V X7R           | C_0805          | (+):+3V3_ESP, (-):GND_STAR
C_ESP_DEC3| 100nF 10V X7R           | C_0805          | (+):+3V3_ESP, (-):GND_STAR

# STATUS LEDs — active-LOW (GPIO sinks current to turn ON)
...
# BOOT_OK: ESP32 firmware sets GPIO1 HIGH after boot complete
# (GPIO1 is also UART TX, firmware switches it after boot)
R_BOOT_OK_PU| 10k 5%               | R_0805          | (1):BOOT_OK, (2):+3V3_ESP
# Note: BOOT_OK feeds into RELAY_EN hardware interlock circuit

# ==============================================================
# SHEET 7: USB-UART + AUTO-RESET — per spec B8
# ==============================================================
...
# AUTO-RESET CIRCUIT — enables automatic firmware programming via USB
Q_RST  | BC847 NPN                  | SOT-23          | B:R_USB_DTR, C:EN_ESP, E:GND_STAR
R_USB_DTR| 1k 5%                     | R_0805          | (1):DTR_USB, (2):Q_RST_B
Q_BOOT | BC847 NPN                  | SOT-23          | B:R_USB_RTS, C:BOOT_ESP, E:GND_STAR
...
U_WD   | TPL5010DDCT                 | SOT-23-6        | VDD:+3V3_ESP, GND:GND_STAR
        ...
        |                             |                  | RESET:WATCHDOG_RST
        |                             |                  | WAKE:NET_WD_WAKE
        |                             |                  | RT:R_WD_RT (timing resistor)
R_WD_DONE_PU| 10k 5%               | R_0805          | (1):WATCHDOG_KICK, (2):+3V3_ESP
R_WD_WAKE| 10k 5%                    | R_0805          | (1):NET_WD_WAKE, (2):GND_STAR
# TIMING RESISTOR — sets watchdog timeout to ~1.6s (T = 1.048576 × R / 32)
# R = 49.9kΩ → T ≈ 1.63s; R = 47kΩ → T ≈ 1.54s
R_WD_RT | 49.9k 1% (E96)           | R_0805          | (1):WD_RT, (2):GND_STAR
# Note: TPL5010 internal 1MΩ gives ~1s default; external R_WD_RT overrides this
C_WD_FILT| 100nF 10V X7R             | C_0805          | (+):+3V3_ESP, (-):GND_STAR
# BAT54S OR-function: prevents voltage conflict between TPL5010 reset and auto-reset circuit
D_RST  | BAT54S                      | SOT-23          | A:WATCHDOG_RST, K:EN_ESP

# ==============================================================
# SHEET 9: 4-CHANNEL RELAY + POWER LIMITING — per spec B9
# ==============================================================

# SHARED RELAY POWER PATH
...
# RELAY 1 (repeat identical for RELAY2/3/4)
Q_R1   | S8050 NPN                   | SOT-23          | B:RELAY1_DRV_via_R_GR1, C:NET_COIL1, E:GND_STAR
...
F_R1_SNUB| 100R 1W + 0.1uF X2 275VAC | 1206/1812     | parallel COM-NO contacts
F_R1_MOV| MOV 275VAC (optional)       | DISC_10MM       | parallel COM-NO contacts
J_R1   | Terminal block 3P 7.62mm | CONN_TB_7.62_3P | Pin1:COM, Pin2:NO, Pin3:NC

# RELAY 2: Q_R2, R_GR2, D_FLY2, K2, F_R2_SNUB, F_R2_MOV, J_R2
# RELAY 3: Q_R3, R_GR3, D_FLY3, K3, F_R3_SNUB, F_R3_MOV, J_R3
# RELAY 4: Q_R4, R_GR4, D_FLY4, K4, F_R4_SNUB, F_R4_MOV, J_R4

# RELAY EN HARDWARE INTERLOCK — per spec B8
# RELAY_EN = firmware GPIO (via R_RELAY_EN_PU) AND POWER_GOOD AND BOOT_OK
# This is a 3-input AND gate implemented with diodes
D_RELAY_PWR_GOOD| BAT54S              | SOT-23          | A:NET_RELAY_INTERLOCK, K:RELAY_EN
D_RELAY_BOOT_OK| BAT54S                | SOT-23          | A:NET_RELAY_INTERLOCK, K:BOOT_OK
R_RELAY_EN_PU| 10k 5%                 | R_0805          | (1):RELAY_EN, (2):+3V3_ESP
R_RELAY_EN_PD| 100k 5%                 | R_0805          | (1):RELAY_EN, (2):GND_STAR
# Note: RELAY_EN must be HIGH for relays to be enabled (pulled HIGH by firmware)
# POWER_GOOD and BOOT_OK are diode-OR'd — both must be HIGH to allow RELAY_EN

# ==============================================================
# SHEET 10: DHT22 + TVS — per spec B10
# ==============================================================

J_DHT  | Terminal block 3P 3.81mm    | CONN_3.81_3P   | 1:+3V3_ANA, 2:GND_STAR, 3:DHT22_RAW
...
J_DS   | Terminal block 3P 3.81mm    | CONN_3.81_3P   | 1:+3V3_ANA, 2:GND_STAR, 3:DS18B20_RAW
...
# ==============================================================
# SHEET 12: SOIL SENSOR + ADS_A3 — per spec B12
# ==============================================================

J_SOIL | Terminal block 3P 3.81mm    | CONN_3.81_3P   | 1:+3V3_ANA, 2:GND_STAR, 3:SOIL_RAW
...
# SOIL_RAW (after R_SOIL_SER) connects directly to ADS1115 A3

# ==============================================================
# SHEET 13: BATTERY/VIN SENSE — per spec B13
# ==============================================================

J_VBAT | Terminal block 2P 5.08mm    | CONN_5.08_2P   | 1:+12V_PROTECTED, 2:GND_STAR
...
# VIN_SENSE_RAW → ESP32 GPIO35 (direct ADC input)
# ADS1115 A0 also reads battery via divider: NET_VIN_DIV → R_A0_SER → ADS_A0
D_VSENSE_CLAMP| BAT54S               | SOT-23          | A:VIN_SENSE_RAW, K:+3V3_ESP
...
U_ADS  | ADS1115IDGSR               | MSOP-8          | VDD:+3V3_ANA, GND:GND_STAR
        ...
        |                             |                  | ALERT:NC (routed to test pad for optional use)
        |                             |                  | A0:ADS_A0 (battery sense via R_A0_SER)
        |                             |                  | A1:ADS_A1 (pH sensor)
        |                             |                  | A2:ADS_A2 (TDS/DO sensor)
        |                             |                  | A3:ADS_A3 (SOIL_RAW)
C_ADS_VDD1| 100nF 16V X7R           | C_0805          | (+):+3V3_ANA, (-):GND_STAR
C_ADS_VDD2| 10uF 16V X7R             | C_0805          | (+):+3V3_ANA, (-):GND_STAR
# ADS1115 uses ESP32 I2C pull-ups (IO21/IO22) — no dedicated pull-ups needed on ADS1115 pins
# All I2C pull-ups consolidated at ESP32 side: 4.7k → +3V3_ANA

R_A0_SER| 1k 5%                      | R_0805          | (1):NET_VIN_DIV, (2):ADS_A0
D_A0_ESD| TVS SMBJ5.0A               | SOD-123         | K:ADS_A0, A:GND_STAR
R_A1_SER| 1k 5%                      | R_0805          | (1):NET_PH_RAW, (2):ADS_A1
D_A1_ESD| TVS SMBJ5.0A               | SOD-123         | K:ADS_A1, A:GND_STAR
R_A2_SER| 1k 5%                      | R_0805          | (1):NET_TDS_RAW, (2):ADS_A2
D_A2_ESD| TVS SMBJ5.0A               | SOD-123         | K:ADS_A2, A:GND_STAR
R_A3_SER| 1k 5%                      | R_0805          | (1):NET_SOIL_SER, (2):ADS_A3
...
# ==============================================================
# SHEET 15: I2C BUS + ESD + SERIES — per spec B15
# ==============================================================
# I2C PULL-UPS: All consolidated at ESP32 side (IO21/IO22 → 4.7k → +3V3_ANA)
# This serves ADS1115, MCP23017×2, BME280, OLED on same bus
...
# Pull-ups at ESP32 I2C pins (IO21/IO22):
R_I2C_PU_SCL| 4.7k 5%              | R_0805          | (1):I2C_SCL, (2):+3V3_ANA
R_I2C_PU_SDA| 4.7k 5%              | R_0805          | (1):I2C_SDA, (2):+3V3_ANA
D_I2C_SCL_ESD| TVS SMBJ5.0A         | SOD-123         | K:I2C_SCL, A:GND_STAR
D_I2C_SDA_ESD| TVS SMBJ5.0A         | SOD-123         | K:I2C_SDA, A:GND_STAR

U_BME280| BME280                     | BME280_I2C      | VCC:+3V3_ESP, GND:GND_STAR, SCL:I2C_SCL, SDA:I2C_SDA
U_OLED | OLED 0.96 I2C             | OLED_I2C        | VCC:+3V3_ESP, GND:GND_STAR, SCL:I2C_SCL, SDA:I2C_SDA

# ==============================================================
# SHEET 16: MicroSD — per spec B16
# ==============================================================
...
# ==============================================================
# SHEET 17: IO EXPANDERS (EXPANSION) — MCP23017×2
# ==============================================================
...
U_EXP1 | MCP23017T-E/SS              | SSOP-28         | VDD:+3V3_ESP, GND:GND_STAR
        ...
        |                             |                  | GPA4..7:EXP1_GPIO4..7 (spare digital I/O)
...
U_EXP2 | MCP23017T-E/SS              | SSOP-28         | VDD:+3V3_ESP, GND:GND_STAR
        ...
        |                             |                  | GPA0..7:EXP2_GPIO0..7 (spare digital I/O)
        |                             |                  | GPB0..7:EXP2_GPIOB0..7 (spare digital I/O)
...
# ==============================================================
# SHEET 18: EXPANSION RELAYS 5-8 — via MCP23017
# ==============================================================

Q_R5   | S8050 NPN                   | SOT-23          | B:EXP1_GPIO0, C:NET_COIL5, E:GND_STAR
...
F_R5_SNUB| 100R 1W + 0.1uF X2 275VAC | 1206/1812     | parallel COM-NO
J_R5   | Terminal block 3P 7.62mm   | CONN_TB_7.62_3P | Pin1:COM, Pin2:NO, Pin3:NC

# RELAY 6: Q_R6, R_GR6, D_FLY6, RELAY6, F_R6_SNUB, J_R6
# RELAY 7: Q_R7, R_GR7, D_FLY7, RELAY7, F_R7_SNUB, J_R7
# RELAY 8: Q_R8, R_GR8, D_FLY8, RELAY8, F_R8_SNUB, J_R8

# ==============================================================
# SHEET 19: DEBUG + TEST POINTS — per spec B17
# ==============================================================
...
TP_I2C_SCL | Test Point | TP | I2C_SCL
TP_I2C_SDA | Test Point | TP | I2C_SDA
...
TP_WD_RST  | Test Point | TP | WATCHDOG_RST
...
D_EXP_ESD1| ESD SOT-23               | SOT-23          | A:EXP1_GPIO4, K:+3V3_ESP
D_EXP_ESD2| ESD SOT-23               | SOT-23          | A:EXP1_GPIO5, K:+3V3_ESP
D_EXP_ESD3| ESD SOT-23               | SOT-23          | A:EXP1_GPIO6, K:+3V3_ESP
D_EXP_ESD4| ESD SOT-23               | SOT-23          | A:EXP1_GPIO7, K:+3V3_ESP
J_MOUNT1..4| M3 Hole 3.2mm           | M3_HOLE         | Mechanical

# ==============================================================
# NET LIST (COMPLETE v3)
# ==============================================================
...
+12V_SURGE         → After R_SURGE + L_SURGE (optional)
...
+3V3_ESP            → After FB_ESP ferrite bead (ESP32, MCP23017, BME280, OLED)
+3V3_ANA            → After FB_ANA ferrite bead (ADS1115, DHT22, DS18B20)
USB_5V              → USB VBUS input
USB_5V_RAW          → After PTC fuse
USB_5V_FUSED        → After R_USB_LIM current limiter
SW_5V               → Buck #1 switching node (shortest trace)
SW_3V3              → Buck #2 switching node (shortest trace)
...
GATE_PROTECT         → P-MOSFET gate (voltage-divider biased)

## SIGNAL NETS
UART0_TX            → ESP32 IO1 → CP2102 RX; also = BOOT_OK (firmware output after boot)
UART0_RX            → ESP32 IO3 ← CP2102 TX
I2C_SCL            → ESP32 IO22 → ALL I2C devices (pull-up 4.7k → +3V3_ANA)
I2C_SDA            → ESP32 IO21 → ALL I2C devices (pull-up 4.7k → +3V3_ANA)
DTR_USB            → CP2102 DTR → Q_RST base
RTS_USB            → CP2102 RTS → Q_BOOT base
WATCHDOG_KICK      → ESP32 IO4 → TPL5010 DONE
WATCHDOG_RST       → TPL5010 RESET → EN_ESP (via BAT54S D_RST OR)
WD_RT              → TPL5010 RT → R_WD_RT → GND (timing resistor)
EN_ESP             → ESP32 enable (from Q_RST OR D_RST BAT54S)
...
DHT22_RAW          → ESP32 IO33 (via 100Ω R_DHT_SER + TVS SMBJ5.0A → GND)
DS18B20_RAW        → ESP32 IO32 (via 100Ω R_DS_SER + TVS SMBJ5.0A → GND)
VIN_SENSE_RAW       → ESP32 IO35 (via 1k R_VSENSE_SER; clamped by BAT54S D_VSENSE_CLAMP)
ADS_A0             → ADS1115 A0 (battery sense, via R_A0_SER 1k + TVS SMBJ5.0A)
ADS_A1             → ADS1115 A1 (pH sensor, via R_A1_SER 1k + TVS SMBJ5.0A)
ADS_A2             → ADS1115 A2 (TDS/DO sensor, via R_A2_SER 1k + TVS SMBJ5.0A)
ADS_A3             → ADS1115 A3 (SOIL_RAW via R_A3_SER 1k + TVS SMBJ5.0A)
NET_VIN_DIV         → Battery voltage divider output (100k/33k → 0–3.3V)
NET_PH_RAW          → pH sensor raw input
NET_TDS_RAW         → TDS/DO sensor raw input
SOIL_RAW            → Soil sensor (J_SOIL-3 → R_SOIL_SER → ADS_A3)
NET_SOIL_SER        → After R_SOIL_SER (filtered, TVS-protected)
SD_CS              → ESP32 IO5 → MicroSD DAT3
SD_SCK              → ESP32 IO18 (via 22Ω R_SD_SCK) → MicroSD CLK
SD_MOSI             → ESP32 IO23 (via 22Ω R_SD_MOSI) → MicroSD CMD
SD_MISO             → ESP32 IO19 (via 22Ω R_SD_MISO) ← MicroSD DAT0
NET_SD_DET          → MicroSD card detect (pull-up to +3V3_ESP)
LED_WIFI_CTRL        → ESP32 IO16 (active-LOW → sinks current to GND)
LED_MQTT_CTRL        → ESP32 IO17 (active-LOW)
LED_ERROR_CTRL       → ESP32 IO13 (active-LOW)
POWER_GOOD           → MCP809T-315 supervisor output → R_LED_PWR + RELAY_EN interlock
RELAY_EN            → Firmware-controlled GPIO (active-HIGH to enable relays)
                     → Diode-OR'd with POWER_GOOD and BOOT_OK for hardware interlock
BOOT_OK             → ESP32 IO1 firmware output (HIGH after boot complete; pull-up 10k)
NET_RELAY_INTERLOCK  → Diode-OR node: (POWER_GOOD OR BOOT_OK) → RELAY_EN

## EXPANSION NETS
EXP1_GPIO0..7        → MCP23017 #1 port A (GPIO0..3 = RELAY5..8, GPIO4..7 = spare)
EXP2_GPIO0..7        → MCP23017 #2 port A (spare)
EXP2_GPIOB0..7       → MCP23017 #2 port B (spare)
EXP1_INTA/B          → Interrupt outputs #1
EXP2_INTA/B          → Interrupt outputs #2
...
NET_WD_WAKE          → TPL5010 WAKE pin (10k pull-down to GND)

## TEST POINT NETS
TP_12V, TP_5V, TP_3V3_ESP, TP_3V3_ANA, TP_GND,
TP_UART_TX, TP_UART_RX, TP_EN, TP_BOOT,
TP_I2C_SCL, TP_I2C_SDA, TP_WD, TP_POWER_GOOD,
TP_RELAY_EN, TP_BOOT_OK, TP_WD_RST
