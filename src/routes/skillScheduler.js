...
router.post('/import', asyncHandler(async (req, res) => {
  const { schedules, mode = 'merge' } = req.body;
  
  if (!schedules || !Array.isArray(schedules)) {
    return res.status(400).json({ error: 'schedules array is required' });
  }
  
  const existingCfg = loadSchedulerConfig();
  const importedSchedules = schedules.map(s => ({
  ...
  let finalSchedules;
  if (mode === 'replace') {
    finalSchedules = importedSchedules;
  } else {
    finalSchedules = [...(existingCfg.schedules || []), ...importedSchedules];
  }
  ...
  logger.info(`[SkillScheduler] Imported ${schedules.length} schedules (mode: ${mode})`);
  res.json({ success: true, count: schedules.length, mode });
}));
