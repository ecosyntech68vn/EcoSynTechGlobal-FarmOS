module.exports = {
  ...
  run: function(ctx) {
    ...
    var weatherHistory = stateStore.get('weatherHistory') || [];
    
    var current = ctx.event.data || {};
    ...
    var recent = weatherHistory.slice(-12);
    var avgTemp = 0;
    var avgHumidity = 0;
    ...
    var predictions = [];
    for (var h = 1; h <= 24; h += 6) {
      var offset = trend === 'warming' ? h * 0.3 : (trend === 'cooling' ? -h * 0.3 : 0);
      predictions.push({
        hoursAhead: h,
